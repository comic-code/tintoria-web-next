...
export default function ContasReceberClient({ type }) {
  const { user } = useAuth();
  ...
  const handleGetDebit = async (client) => {
    if (!client) return ErrorAlert("Selecione um cliente!");
    ...
    try {
      ...
      if (response.error) {
        ...
        const { triggerBase64Download } = await import("common-base64-downloader-react");
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        console.error(err);
        ErrorAlert("Erro ao carregar clientes!", err.message);
      }
    };
